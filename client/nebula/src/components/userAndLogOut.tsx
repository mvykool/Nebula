...
const UserAndLogOut = () => {
  ...
  return (
    <div className="relative">
      <div className="flex cursor-pointer items-center gap-3 hover:bg-gray-800 pr-14 rounded-md" onClick={() => setUsername(!username)}>
      ...
      {username
        ? <div className="border boder-white absolute w-full rounded-b-md py-5">
          <button className="flex items-center gap-2 ml-3" onClick={() => auth.logOut()}>Logout <i className='bx bx-exit'></i></button>
        </div>
        : null
      }
    </div>
